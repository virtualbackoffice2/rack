...
const DEMO_ACCOUNT_MESSAGE = "This is a demo account!";
...
function showDemoAccountToast() {
  showToast(DEMO_ACCOUNT_MESSAGE);
}

function blockDemoAction(e) {
  if (e) {
    e.preventDefault();
    e.stopPropagation();
  }
  showDemoAccountToast();
}

function lockDemoEditor(root) {
  const remarkInput = root.querySelector(".remarkInput");
  if (remarkInput) {
    remarkInput.readOnly = true;
    remarkInput.title = DEMO_ACCOUNT_MESSAGE;
    remarkInput.classList.add("demo-readonly");
    remarkInput.addEventListener("focus", () => remarkInput.blur());
    remarkInput.addEventListener("click", showDemoAccountToast);
  }

  const modeSelect = root.querySelector(".modeSel");
  if (modeSelect) {
    modeSelect.disabled = true;
    modeSelect.title = DEMO_ACCOUNT_MESSAGE;
    modeSelect.classList.add("demo-disabled");
  }

  const teamSelect = root.querySelector(".teamSel");
  if (teamSelect) {
    teamSelect.disabled = true;
    teamSelect.title = DEMO_ACCOUNT_MESSAGE;
    teamSelect.classList.add("demo-disabled");
  }
}
...
function renderSingleCard(r, index, container) {
  ...
  markBtn.title = DEMO_ACCOUNT_MESSAGE;
  markBtn.onclick = blockDemoAction;
  ...
  removeBtn.title = DEMO_ACCOUNT_MESSAGE;
  removeBtn.onclick = blockDemoAction;
